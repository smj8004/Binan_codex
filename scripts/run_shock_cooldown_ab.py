...
FIXED_LOOKBACK_BARS = 24 * 7  # median_3 base: 7d -> (7d,14d,28d)
FIXED_K = 4
FIXED_RANK_BUFFER = 2
...
def _run_case(
    *,
    ...
    config_cooldown = int(run_cfg.get("safety", {}).get("shock_cooldown_bars", -1))
    diagnostics_cooldown = int(diagnostics.get("shock_cooldown_bars", -1))
    if config_cooldown != int(shock_cooldown_bars):
        raise RuntimeError(f"{scenario}: config shock_cooldown_bars mismatch ({config_cooldown} != {shock_cooldown_bars}).")
    if diagnostics_cooldown != int(shock_cooldown_bars):
        raise RuntimeError(f"{scenario}: diagnostics shock_cooldown_bars mismatch ({diagnostics_cooldown} != {shock_cooldown_bars}).")
    ...
    row = {
        ...
        "config_shock_cooldown_bars": int(config_cooldown),
        "diagnostics_shock_cooldown_bars": int(diagnostics_cooldown),
    }
...
def main() -> None:
    ...
    sweep_dir = out_root / f"shock_cooldown_ab_{sweep_id}"
    ...
    run_a = _run_case(
        ...
        scenario="A_cooldown_72",
        shock_cooldown_bars=SHOCK_COOLDOWN_A,
    )
    run_b = _run_case(
        out_root=out_root,
        baseline_cfg=baseline_cfg,
        initial_equity=initial_equity,
        scenario="B_cooldown_48",
        shock_cooldown_bars=SHOCK_COOLDOWN_B,
    )
    ...
    report_lines = [
        f"# Shock Cooldown A/B Report ({sweep_id})",
        "",
        "## Scope",
        f"- guide selected baseline run id: `{selected_run_id}`",
        "- single lever only: `shock_cooldown_bars`",
        f"- Run A: `shock_cooldown_bars={SHOCK_COOLDOWN_A}` (baseline)",
        f"- Run B: `shock_cooldown_bars={SHOCK_COOLDOWN_B}` (variant)",
        "- all other fixed values unchanged (`testnet=False`, `k=4`, `rank_buffer=2`, `lookback_score_mode=median_3`, `shock_mode=downweight`, `shock_freeze_min_fraction=0.40`, extreme/delever fixed, cost model unchanged)",
        "",
        "## Run IDs",
        ...
        f"- Run A config testnet: `{run_a['config_testnet']}`",
        f"- Run A config/diagnostics shock_cooldown_bars: `{run_a['config_shock_cooldown_bars']}` / `{run_a['diagnostics_shock_cooldown_bars']}`",
        f"- Run B config testnet: `{run_b['config_testnet']}`",
        f"- Run B config/diagnostics shock_cooldown_bars: `{run_b['config_shock_cooldown_bars']}` / `{run_b['diagnostics_shock_cooldown_bars']}`",
        "",
        "## Baseline vs Variant",
        "| scenario | run_id | shock_cooldown_bars | net_pnl | max_drawdown | fee_cost_total | oos_positive_ratio | avg_turnover_ratio | skipped_ratio | avg_effective_gross | shock_skip_ratio | extreme_skip_ratio | extreme_no_trade_ratio | liquidation_count | eq0_count |",
        "|---|---|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|",
    ]
    for row in comparison_df.to_dict(orient="records"):
        report_lines.append(
            f"| {row['scenario']} | {row['run_id']} | {int(row['shock_cooldown_bars'])} | "
            ...
            f"{float(row['extreme_no_trade_ratio']):.6f} | {float(row['liquidation_count']):.0f} | {float(row['equity_zero_or_negative_count']):.0f} |"
        )
    report_lines.extend(
        [
            "",
    ...
    print(f"VERIFY A testnet={run_a['config_testnet']} config_cooldown={run_a['config_shock_cooldown_bars']} diagnostics_cooldown={run_a['diagnostics_shock_cooldown_bars']}")
    print(f"VERIFY B testnet={run_b['config_testnet']} config_cooldown={run_b['config_shock_cooldown_bars']} diagnostics_cooldown={run_b['diagnostics_shock_cooldown_bars']}")
    print(str(sweep_dir))
