...
def _run_case(
    *,
    ...
    config_gap_thr = float(run_cfg.get("safety", {}).get("gap_shock_threshold", -1.0))
    diagnostics_gap_thr = float(diagnostics.get("gap_shock_threshold", -1.0))
    if abs(config_gap_thr - float(gap_shock_threshold)) > 1e-12:
        raise RuntimeError(f"{scenario}: config gap_shock_threshold mismatch ({config_gap_thr} != {gap_shock_threshold}).")
    if abs(diagnostics_gap_thr - float(gap_shock_threshold)) > 1e-12:
        raise RuntimeError(f"{scenario}: diagnostics gap_shock_threshold mismatch ({diagnostics_gap_thr} != {gap_shock_threshold}).")
    ...
    row = {
        ...
        "config_gap_thr": float(config_gap_thr),
        "diagnostics_gap_thr": float(diagnostics_gap_thr),
    }
...
def main() -> None:
    ...
    sweep_dir = out_root / f"gap_shock_threshold_ab_{sweep_id}"
    ...
    report_lines = [
        f"# GAP Shock Threshold A/B Report ({sweep_id})",
        "",
        "## Scope",
        f"- guide selected baseline run id: `{selected_run_id}`",
        "- single lever only: `gap_shock_threshold`",
        f"- Run A: `gap_shock_threshold={GAP_THR_A:.2f}` (baseline)",
        f"- Run B: `gap_shock_threshold={GAP_THR_B:.2f}` (variant)",
        f"- fixed: `shock_cooldown_bars={FIXED_SHOCK_COOLDOWN_BARS}`, `atr_shock_threshold={FIXED_ATR_SHOCK_THRESHOLD}` and all other baseline fixed values unchanged",
        "",
        "## Run IDs",
        ...
        f"- VERIFY A testnet={run_a['config_testnet']} config_gap_thr={run_a['config_gap_thr']:.2f} diagnostics_gap_thr={run_a['diagnostics_gap_thr']:.2f}",
        f"- VERIFY B testnet={run_b['config_testnet']} config_gap_thr={run_b['config_gap_thr']:.2f} diagnostics_gap_thr={run_b['diagnostics_gap_thr']:.2f}",
        "",
        "## Baseline vs Variant",
        "| scenario | run_id | gap_shock_threshold | net_pnl | max_drawdown | fee_cost_total | oos_positive_ratio | avg_turnover_ratio | skipped_ratio | avg_effective_gross | shock_skip_ratio | extreme_skip_ratio | extreme_no_trade_ratio | atr_shock_count | gap_shock_count | liquidation_count | eq0_count |",
        "|---|---|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|",
    ]
    for row in comparison_df.to_dict(orient="records"):
        report_lines.append(
            f"| {row['scenario']} | {row['run_id']} | {float(row['gap_shock_threshold']):.2f} | "
    ...
    print(f"VERIFY A testnet={run_a['config_testnet']} config_gap_thr={run_a['config_gap_thr']:.2f} diagnostics_gap_thr={run_a['diagnostics_gap_thr']:.2f}")
    print(f"VERIFY B testnet={run_b['config_testnet']} config_gap_thr={run_b['config_gap_thr']:.2f} diagnostics_gap_thr={run_b['diagnostics_gap_thr']:.2f}")
    print(str(sweep_dir))
