...
def _run_case(
    *,
    ...
    config_gap_mult = float(run_cfg.get("safety", {}).get("shock_weight_mult_gap", -1.0))
    diagnostics_gap_mult = float(diagnostics.get("shock_weight_mult_gap", -1.0))
    if abs(config_gap_mult - float(shock_weight_mult_gap)) > 1e-12:
        raise RuntimeError(f"{scenario}: config shock_weight_mult_gap mismatch ({config_gap_mult} != {shock_weight_mult_gap}).")
    if abs(diagnostics_gap_mult - float(shock_weight_mult_gap)) > 1e-12:
        raise RuntimeError(f"{scenario}: diagnostics shock_weight_mult_gap mismatch ({diagnostics_gap_mult} != {shock_weight_mult_gap}).")
    ...
    row = {
        ...
        "config_gap_mult": float(config_gap_mult),
        "diagnostics_gap_mult": float(diagnostics_gap_mult),
    }
...
def main() -> None:
    ...
    sweep_dir = out_root / f"shock_weight_mult_gap_ab_{sweep_id}"
    ...
    run_a = _run_case(
        ...
        scenario="A_gap_mult_0.10",
        shock_weight_mult_gap=GAP_MULT_A,
    )
    run_b = _run_case(
        out_root=out_root,
        baseline_cfg=baseline_cfg,
        initial_equity=initial_equity,
        scenario="B_gap_mult_0.15",
        shock_weight_mult_gap=GAP_MULT_B,
    )

    comparison_cols = [
        "scenario",
        "run_id",
        "shock_weight_mult_gap",
        "net_pnl",
        "max_drawdown",
        "fee_cost_total",
        "oos_positive_ratio",
    ...
    report_lines = [
        f"# Shock Weight Gap Multiplier A/B Report ({sweep_id})",
        "",
        "## Scope",
        f"- guide selected baseline run id: `{selected_run_id}`",
        "- single lever only: `shock_weight_mult_gap`",
        f"- Run A: `shock_weight_mult_gap={GAP_MULT_A:.2f}` (baseline)",
        f"- Run B: `shock_weight_mult_gap={GAP_MULT_B:.2f}` (variant)",
        f"- fixed: `shock_cooldown_bars={FIXED_SHOCK_COOLDOWN_BARS}`, `atr_shock_threshold={FIXED_ATR_SHOCK_THRESHOLD}`, `gap_shock_threshold={FIXED_GAP_SHOCK_THRESHOLD:.2f}` and all other baseline fixed values unchanged",
        "",
        "## Run IDs",
        ...
        f"- VERIFY A testnet={run_a['config_testnet']} config_gap_mult={run_a['config_gap_mult']:.2f} diagnostics_gap_mult={run_a['diagnostics_gap_mult']:.2f}",
        f"- VERIFY B testnet={run_b['config_testnet']} config_gap_mult={run_b['config_gap_mult']:.2f} diagnostics_gap_mult={run_b['diagnostics_gap_mult']:.2f}",
        "",
        "## Baseline vs Variant",
        "| scenario | run_id | shock_weight_mult_gap | net_pnl | max_drawdown | fee_cost_total | oos_positive_ratio | avg_turnover_ratio | skipped_ratio | avg_effective_gross | shock_skip_ratio | extreme_skip_ratio | extreme_no_trade_ratio | gap_shock_count | atr_shock_count | liquidation_count | eq0_count |",
        "|---|---|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|",
    ]
    for row in comparison_df.to_dict(orient="records"):
        report_lines.append(
            f"| {row['scenario']} | {row['run_id']} | {float(row['shock_weight_mult_gap']):.2f} | "
            ...
            f"{float(row['extreme_no_trade_ratio']):.6f} | {float(row['gap_shock_count']):.0f} | {float(row['atr_shock_count']):.0f} | "
    ...
    print(f"VERIFY A testnet={run_a['config_testnet']} config_gap_mult={run_a['config_gap_mult']:.2f} diagnostics_gap_mult={run_a['diagnostics_gap_mult']:.2f}")
    print(f"VERIFY B testnet={run_b['config_testnet']} config_gap_mult={run_b['config_gap_mult']:.2f} diagnostics_gap_mult={run_b['diagnostics_gap_mult']:.2f}")
    print(str(sweep_dir))
