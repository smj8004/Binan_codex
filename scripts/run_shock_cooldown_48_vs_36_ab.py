...
FIXED_SHOCK_WEIGHT_MULT_ATR = 0.30
...
def _run_case(
    *,
    ...
    output = run_portfolio_validation(
        ...
        shock_weight_mult_atr=float(FIXED_SHOCK_WEIGHT_MULT_ATR),
    ...
    if config_cooldown != diagnostics_cooldown:
        raise RuntimeError(f"{scenario}: config/diagnostics cooldown mismatch ({config_cooldown} != {diagnostics_cooldown}).")
    if config_cooldown != int(shock_cooldown_bars):
        raise RuntimeError(f"{scenario}: cooldown does not match requested value ({config_cooldown} != {shock_cooldown_bars}).")
    ...
    row = {
        ...
        "config_cooldown": int(config_cooldown),
        "diagnostics_cooldown": int(diagnostics_cooldown),
    }
...
def main() -> None:
    ...
    sweep_dir = out_root / f"shock_cooldown_48_vs_36_ab_{sweep_id}"
    ...
    report_lines = [
        f"# Shock Cooldown 48 vs 36 A/B Report ({sweep_id})",
        "",
        "## Scope",
        ...
        "- fixed: `testnet=False`, `k=4`, `rank_buffer=2`, `lookback_score_mode=median_3`, `shock_freeze_min_fraction=0.40`, `atr_shock_threshold=2.7`, `gap_shock_threshold=0.12`, `shock_weight_mult_gap=0.15`, `shock_weight_mult_atr=0.30`, extreme/delever fixed",
        "",
        "## Run IDs",
        ...
        f"- VERIFY A testnet={run_a['config_testnet']} config_cooldown={run_a['config_cooldown']} diagnostics_cooldown={run_a['diagnostics_cooldown']}",
        f"- VERIFY B testnet={run_b['config_testnet']} config_cooldown={run_b['config_cooldown']} diagnostics_cooldown={run_b['diagnostics_cooldown']}",
        "",
        "## Baseline vs Variant",
        "| scenario | run_id | shock_cooldown_bars | net_pnl | max_drawdown | fee_cost_total | oos_positive_ratio | avg_turnover_ratio | skipped_ratio | avg_effective_gross | shock_skip_ratio | extreme_skip_ratio | extreme_no_trade_ratio | atr_shock_count | gap_shock_count | liquidation_count | eq0_count |",
        "|---|---|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|",
    ]
    ...
    print(f"VERIFY A testnet={run_a['config_testnet']} config_cooldown={run_a['config_cooldown']} diagnostics_cooldown={run_a['diagnostics_cooldown']}")
    print(f"VERIFY B testnet={run_b['config_testnet']} config_cooldown={run_b['config_cooldown']} diagnostics_cooldown={run_b['diagnostics_cooldown']}")
    print(str(sweep_dir))
