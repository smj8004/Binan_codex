...
LOOKBACK_A_MODE = "median_3"
LOOKBACK_A_BARS = 24 * 7  # 7d base -> median of (7d,14d,28d)
LOOKBACK_B_MODE = "single"
LOOKBACK_B_BARS = 24 * 28  # exact 28d when mode is single
...
def _run_case(
    *,
    ...
    output = run_portfolio_validation(
        ...
        lookback_score_mode=str(lookback_mode),
...
def _pick_winner(run_a: dict[str, Any], run_b: dict[str, Any]) -> tuple[str, str]:
    ...
    diff_ratio = abs(a_pnl - b_pnl) / max(abs(a_pnl), abs(b_pnl), 1e-9)
    if diff_ratio <= 0.05:
        ...
        a_oos = float(run_a["oos_positive_ratio"])
        b_oos = float(run_b["oos_positive_ratio"])
        if b_oos > a_oos:
            return "B", "net_pnl and max_drawdown are tied; B has higher oos_positive_ratio."
        if a_oos > b_oos:
            return "A", "net_pnl and max_drawdown are tied; A has higher oos_positive_ratio."
        return "TIE", "All primary comparison metrics are tied."
    ...
    b_oos = float(run_b["oos_positive_ratio"])
    if b_oos > a_oos:
        return "B", "net_pnl tied; B has higher oos_positive_ratio."
    if a_oos > b_oos:
        return "A", "net_pnl tied; A has higher oos_positive_ratio."
    return "TIE", "net_pnl and oos_positive_ratio are tied."
...
def main() -> None:
    ...
    sweep_dir = out_root / f"lookback_mode_ab_{sweep_id}"
    ...
    run_a = _run_case(
        ...
        lookback_mode=LOOKBACK_A_MODE,
        lookback_bars=LOOKBACK_A_BARS,
        scenario="A_median_3_7_14_28",
    )
    run_b = _run_case(
        out_root=out_root,
        baseline_cfg=baseline_cfg,
        initial_equity=initial_equity,
        lookback_mode=LOOKBACK_B_MODE,
        lookback_bars=LOOKBACK_B_BARS,
        scenario="B_single_28d",
    )
    ...
    report_lines = [
        f"# Lookback Mode A/B Report ({sweep_id})",
        "",
        "## Scope",
        f"- guide selected baseline run id: `{selected_run_id}`",
        "- fixed baseline copied from guide/BASELINE_STATE.md (`testnet=False`, `k=4`, `rank_buffer=2`, extreme/delever stack fixed, `shock_freeze_min_fraction=0.40`)",
        "- single lever only: lookback scoring mode",
        f"- Run A: `lookback_score_mode={LOOKBACK_A_MODE}`, `lookback_bars={LOOKBACK_A_BARS}` (median on 7d/14d/28d)",
        f"- Run B: `lookback_score_mode={LOOKBACK_B_MODE}`, `lookback_bars={LOOKBACK_B_BARS}` (single 28d)",
        "",
        "## Run IDs",
        ...
        "",
        "## Baseline vs Variant",
        "| scenario | run_id | lookback_score_mode | lookback_bars | net_pnl | max_drawdown | fee_cost_total | oos_positive_ratio | avg_turnover_ratio | skipped_ratio | avg_effective_gross | liquidation_count | eq0_count |",
        "|---|---|---|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|",
    ]
    for row in comparison_df.to_dict(orient="records"):
        report_lines.append(
            f"| {row['scenario']} | {row['run_id']} | {row['lookback_score_mode']} | {int(row['lookback_bars'])} | "
            ...
            f"{float(row['avg_effective_gross']):.6f} | {float(row['liquidation_count']):.0f} | {float(row['equity_zero_or_negative_count']):.0f} |"
        )
    report_lines.extend(
        [
            "",
