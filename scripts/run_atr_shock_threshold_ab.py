...
def _run_case(
    *,
    ...
    config_atr_thr = float(run_cfg.get("safety", {}).get("atr_shock_threshold", -1.0))
    diagnostics_atr_thr = float(diagnostics.get("atr_shock_threshold", -1.0))
    if abs(config_atr_thr - float(atr_shock_threshold)) > 1e-12:
        raise RuntimeError(f"{scenario}: config atr_shock_threshold mismatch ({config_atr_thr} != {atr_shock_threshold}).")
    if abs(diagnostics_atr_thr - float(atr_shock_threshold)) > 1e-12:
        raise RuntimeError(f"{scenario}: diagnostics atr_shock_threshold mismatch ({diagnostics_atr_thr} != {atr_shock_threshold}).")
    ...
    row = {
        ...
        "config_atr_thr": float(config_atr_thr),
        "diagnostics_atr_thr": float(diagnostics_atr_thr),
    }
...
def main() -> None:
    ...
    sweep_dir = out_root / f"atr_shock_threshold_ab_{sweep_id}"
    ...
    report_lines = [
        f"# ATR Shock Threshold A/B Report ({sweep_id})",
        "",
        "## Scope",
        f"- guide selected baseline run id: `{selected_run_id}`",
        "- single lever only: `atr_shock_threshold`",
        f"- Run A: `atr_shock_threshold={ATR_THR_A}` (baseline)",
        f"- Run B: `atr_shock_threshold={ATR_THR_B}` (variant)",
        f"- fixed: `shock_cooldown_bars={FIXED_SHOCK_COOLDOWN_BARS}` and all other baseline fixed values unchanged",
        "",
        "## Run IDs",
        ...
        f"- VERIFY A testnet={run_a['config_testnet']} config_atr_thr={run_a['config_atr_thr']} diagnostics_atr_thr={run_a['diagnostics_atr_thr']}",
        f"- VERIFY B testnet={run_b['config_testnet']} config_atr_thr={run_b['config_atr_thr']} diagnostics_atr_thr={run_b['diagnostics_atr_thr']}",
        "",
        "## Baseline vs Variant",
        "| scenario | run_id | atr_shock_threshold | net_pnl | max_drawdown | fee_cost_total | oos_positive_ratio | avg_turnover_ratio | skipped_ratio | avg_effective_gross | shock_skip_ratio | extreme_skip_ratio | extreme_no_trade_ratio | atr_shock_count | gap_shock_count | liquidation_count | eq0_count |",
        "|---|---|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|",
    ]
    for row in comparison_df.to_dict(orient="records"):
        report_lines.append(
            f"| {row['scenario']} | {row['run_id']} | {float(row['atr_shock_threshold']):.1f} | "
    ...
    print(f"VERIFY A testnet={run_a['config_testnet']} config_atr_thr={run_a['config_atr_thr']} diagnostics_atr_thr={run_a['diagnostics_atr_thr']}")
    print(f"VERIFY B testnet={run_b['config_testnet']} config_atr_thr={run_b['config_atr_thr']} diagnostics_atr_thr={run_b['diagnostics_atr_thr']}")
    print(str(sweep_dir))
